...
with TelegramClient(sesi_file, api_id, api_hash) as client:
        ...
        async def handler(event):
            ...
            if 'Ada enam' in event.raw_text:
                time.sleep(2)
                await event.click(text="Play 10x (Bonus 1)")
                return

            if 'Kamu memutar SlotMachine' in event.raw_text:
                time.sleep(2)
            ...
            elif 'Kumpulkan' in event.raw_text:
                time.sleep(2)
                await event.respond(cmd)
                return

            elif 'mengumpulkan' in event.raw_text:
                time.sleep(2)
                await event.respond('/th_SlotMachine_SixLeaves')
                return
